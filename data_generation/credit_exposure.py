...
def short_rate(nsim: int, seq: ql.GaussianPathGenerator, 
               nbr_gridpoints: int):
    """
...
def zcb_price(rate: np.ndarray, T: int, zero_rates: list,
              fwd_rates: list, gridpoints: pd.Series, nsim: int, 
              a: float, sigma: float):
    """
    ...
    lnA = np.log(P_T / P_t) + B * fwd_rates[0: T+1] - \
          ((0.25 * sigma ** 2) / (a * a * a)) * \
          ((np.exp(-a * t[T]) - np.exp(-a * t[0: T+1])) ** 2) * \
          (np.exp(2 * a * t[0: T+1]) - 1)
    A = np.exp(lnA)
    A = np.transpose(np.tile(A, (nsim, 1)))
    B = np.transpose(np.tile(B, (nsim, 1)))
    s = np.append(0,gridpoints[0:T])
    M = -(sigma**2/a**2)*(1-np.exp(-a*(t-s))) + \
        (sigma**2/(2*a**2))*(np.exp(-a * (t[T] - \
        t[0:T + 1])) - np.exp(-a * (t[T] + t[0:T + 1] - 2*s)))
    M = np.transpose(np.tile(M, (nsim, 1)))
    P_t_T = A * np.exp(-B * (short_rates + M))
    return P_t_T


def irs(rate: np.ndarray, T: int,  zero_rates, fwd_rates, 
        gridpoints: np.ndarray, nsim: int, a: float, sigma: float, 
        strike: float, delta=0.5, fl_freq: int=3, N: int=100):

    """
    Returns values of a recieve fixed play floating interest rate 
    swap (nbr_gridpoints x nsim).
    ...
    ARGS:
        ...
        a (float):                        HW1F param mean reversion.
        sigma (float):                    HW1F param volatility.
        strike (float):                   strike in bps.
        delta (float):                    time delta fixed (years).
        fl_freq (float):                  time delta float (months).
    ...
    zcb_prices = [
        zcb_price(rate, i, zero_rates, fwd_rates, gridpoints, nsim,
                  a, sigma) 
        for i in tenor
    ]
    libor_rates = [
        1 / (zcb_prices[i+1][tenor[i],:]) 
    ...
    R = (all_floating / all_fix) + strike*0.0001
    V = np.empty(shape=(0, nsim))
    for j in range(0,nbr_payments):
        outstand_fix = list(filter((tenor[j]).__lt__, tenor_fix))
        outstand_ind = [tenor.index(i) for i in outstand_fix]
        outstand_fl = list(filter((tenor[j]).__le__, tenor))
        outstand_ind_fl = [tenor.index(i) for i in outstand_fl][1:]
        l = outstand_ind_fl[0]
        fix_leg = sum(R * delta *
                      zcb_prices[l][tenor_idx[j]:tenor_idx[j+1], :]
                      for l in outstand_ind)
        fl_leg = libor_rates[l-1] * \
                 zcb_prices[l][tenor_idx[j]:tenor_idx[j+1], :] - \
                 zcb_prices[-1][tenor_idx[j]:tenor_idx[j+1], :]
...
def exposure(y0: np.ndarray, dates: np.ndarray, T: int, a: float, 
             sigma: float, nsim: int, dayCounter: ql.ActualActual, 
             nbr_gridpoints: int, strike: float, N):
    """
    Returns pair of yield curve & expected positive exposure.
    ...
    ARGS:
        ...
        sigma (float):                    HW1F param volatility.
        nsim (int):                       nbr of MC simulations.
        dayCounter (ql.ActualActual):     daycount object.
        nbr_gridpoints (int):             nbr of time gridpoints.
        strike (float):                   deviation from ATM-strike.
        N (int):                          notional principal (M USD)
    """
    ...
    all_dates_pd = pd.DateOffset(days=start_date.day-1) + \
                   pd.Series(
        pd.date_range(start_date - pd.DateOffset(days=start_date.day),
                      end_date,
                      freq='MS').tolist())
    ...
    rng = ql.GaussianRandomSequenceGenerator(
        ql.UniformRandomSequenceGenerator(nbr_gridpoints-1, 
                                        ql.UniformRandomGenerator()))
    seq = ql.GaussianPathGenerator(hw_process, T, 
                                   nbr_gridpoints-1, rng, False)
    t0 = time.time()
    rate = short_rate(nsim, seq, nbr_gridpoints)
    t1 = time.time()
    t2 = time.time()
    irs_values = irs(rate, T, zero_rates, fwd_rates, gridpoints, 
                     nsim, a, sigma, strike, N)
